...
(async()=>{
 ...
 while(Date.now()-started<240000){
  ...
  for(const [name,threshold] of [['approach',1400],['descent',1200],['valley',700],['first-sam',300],['bandit',-500],['bend',-4500],['reveal',-5200],['attack',-5900],['escape',-7000]])if(z<threshold&&!captured.has(name)){captured.add(name);await page.screenshot({path:path.join(output,name+'.png')});}
  ...
  if(z<-5000&&z>-6600&&!s.destroyed)targetX=s.target[0];
  let altitude=z>3500?110:50;
  let desiredY=s.aheadFloor+altitude;
  if(z<-5400&&!s.destroyed)desiredY=s.target[1]+100;
  ...
  if((z<-2400&&z>-4300)||(s.destroyed&&z<-7000))next.add('Shift');
