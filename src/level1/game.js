...
function hostileLockSolution(){if((worldIndex===0&&!mission.destroyed)||enemyRole!=='ACE'||!enemyAlive||crashed||missionComplete||missionCompleteTimer>0||enemyTime<1.15)return false;const toShip=ship.position.clone().sub(enemy.position),range=toShip.length();if(range<130||range>680)return false;const forward=new THREE.Vector3(0,0,-1).applyQuaternion(enemy.quaternion).normalize();return forward.dot(toShip.multiplyScalar(1/Math.max(range,.001)))>.72&&enemyLOS()}
