...
const ship=makeShip(),enemy=makeShip(true);ship.scale.setScalar(1.12);enemy.scale.setScalar(1.55);scene.add(ship,enemy);for(const m of scenery)place(m,true);
...
function updateCamera(dt){
  ...
  camBank=THREE.MathUtils.lerp(camBank,Math.sin(bank)*.16,1-Math.exp(-dt/.11));
  camPos.copy(ship.position).addScaledVector(camF,-17.5-velocity*5.5-pull*2-Math.max(0,1/camera.aspect-1)*12).addScaledVector(worldUp,6.3+pull*.8);
  ...
  look.copy(ship.position).addScaledVector(camF,56+velocity*14+pull*8).addScaledVector(worldUp,-8.5+pull*3.5);
  ...
  camera.fov=THREE.MathUtils.lerp(camera.fov,68+velocity*18+pull*4+groundRush*3+gunKick*.6+hitKick*.4,1-Math.exp(-dt/.11));
  camera.updateProjectionMatrix();sky.position.copy(camera.position);
}

function resetCameraFrame(){viewRight.set(1,0,0);camBank=0;camera.quaternion.identity();camera.fov=68;camera.lookAt(look.copy(ship.position).add(new THREE.Vector3(0,-8.5,-56)));}
