...
updateEnemy=function(dt){
 ...
 if(duel.state==='extend'&&duel.age>(enemyRole==='ACE'?.48:(firstTarget?3.8:3.1))&&(range>(enemyRole==='ACE'?180:420)||duel.age>(enemyRole==='ACE'?1.1:9)))duelState('engage');
...
function samExposure(s){
 ...
 const clutter=.34+.66*THREE.MathUtils.smoothstep(agl,16,90);
 const centerLane=1+1.15*(1-THREE.MathUtils.smoothstep(crossTrack,110,310));
 const terrainTuck=.64+.36*THREE.MathUtils.smoothstep(crossTrack,210,430);
 const highExposure=1+THREE.MathUtils.smoothstep(agl,100,185)*1.55;
