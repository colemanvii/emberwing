...
const ship=makeShip(),enemy=makeShip(true);scene.add(ship,enemy);for(const m of scenery)place(m,true);
...
updateSpeedFX=function(dt){
  cinematicSpeedBase(dt);
  const skim=1-THREE.MathUtils.smoothstep(ship.position.y-terrainHeight(ship.position.x,ship.position.z),18,95);
  speedMat.opacity*=.18+skim*.20;
...
updateFlight=function(dt){
 ...
 pitchRate=THREE.MathUtils.lerp(pitchRate,pi*1.4,1-Math.exp(-dt/.13));
 rollRate=THREE.MathUtils.lerp(rollRate,ri*2.6,1-Math.exp(-dt/.07));
 ...
 const pullBlend=THREE.MathUtils.clamp(Math.abs(pitchRate)/1.4,0,1);
 const coordinationGain=THREE.MathUtils.lerp(1,.28,pullBlend);
 if(horizontal>.001&&coordinationGain>.001){
  flightStep.setFromAxisAngle(worldUp,-(Math.sin(bank)*.72+ri*.22)*horizontal*dt*coordinationGain);
...
function launchSam(site){if(sam.missiles.length>=(mission.destroyed?2:1)||missionElapsed-sam.lastLaunch<3.2)return;samLaunchBurst(site);const m=new THREE.Group(),body=new THREE.Mesh(new THREE.CylinderGeometry(.19,.26,3.2,8),new THREE.MeshStandardMaterial({color:0xc9c5b6,metalness:.38,roughness:.44}));body.rotation.x=Math.PI/2;m.add(body);const nose=new THREE.Mesh(new THREE.ConeGeometry(.25,.75,8),new THREE.MeshBasicMaterial({color:0xff633a}));nose.rotation.x=-Math.PI/2;nose.position.z=-1.95;m.add(nose);const flame=new THREE.Mesh(new THREE.ConeGeometry(.23,2.1,8),new THREE.MeshBasicMaterial({color:0xff6b31,transparent:true,opacity:.94,blending:THREE.AdditiveBlending,depthWrite:false}));flame.rotation.x=-Math.PI/2;flame.position.z=2.45;m.add(flame);const start=site.position.clone().addScaledVector(worldUp,4),initial=ship.position.clone().addScaledVector(worldUp,95).sub(start).normalize();m.position.copy(start);m.quaternion.setFromUnitVectors(new THREE.Vector3(0,0,-1),initial);scene.add(m);const h={mesh:m,v:initial.multiplyScalar(238),life:8,age:0,lostTrack:false,trail:0,warn:0,smoke:0,near:false,site};sam.missiles.push(h);syncSamMissileAlias();sam.lastLaunch=missionElapsed;site.lock=0;site.stage=0;site.hotUntil=missionElapsed+2.4;site.cooldown=mission.destroyed?1.05:(site.index===2?6.5:1.55);sam.site=site;announce('SAM LAUNCH — '+clockBearing(start)+" O'CLOCK");flashScreen(.1);chirp(1060,.07,.045);chirp(1450,.11,.04,.07)}
function updateOneSamMissile(h,dt){h.life-=dt;h.age+=dt;h.trail-=dt;h.warn-=dt;h.smoke-=dt;if(h.warn<=0){chirp(960,.04,.024);h.warn=.36}if(h.trail<=0){spawnMissileTrail(h.mesh.position,h.v);h.trail=.035}if(h.smoke<=0){spawnV43SamSmoke(h.mesh.position,h.v);h.smoke=.065}const previous=h.mesh.position.clone();
 // A finite turn radius lets a committed break beat the missile. Once it passes
 // the pilot outside its seeker cone, it continues ballistically instead of circling back.
 const toPlayer=ship.position.clone().sub(previous),range=toPlayer.length();
 const direction=h.v.clone().normalize();
 if(h.age>.9&&range<260&&direction.dot(toPlayer.clone().normalize())<.42)h.lostTrack=true;
 if(!h.lostTrack){
  const lead=ship.position.clone().addScaledVector(new THREE.Vector3(0,0,-1).applyQuaternion(ship.quaternion),speed*.09);
  const desired=lead.sub(previous).normalize(),angle=direction.angleTo(desired);
  const turn=(h.age<.55?.45:(mission.destroyed?1.05:.9))*dt;
  if(angle>1e-6)direction.applyAxisAngle(new THREE.Vector3().crossVectors(direction,desired).normalize(),Math.min(angle,turn));
 }
 h.v.copy(direction).multiplyScalar(THREE.MathUtils.lerp(h.v.length(),mission.destroyed?326:308,1-Math.exp(-dt*2)));
 h.mesh.quaternion.setFromUnitVectors(new THREE.Vector3(0,0,-1),h.v.clone().normalize());h.mesh.position.addScaledVector(h.v,dt);const travel=h.mesh.position.clone().sub(previous),toShip=ship.position.clone().sub(previous),u=THREE.MathUtils.clamp(toShip.dot(travel)/Math.max(.001,travel.lengthSq()),0,1),closest=previous.clone().addScaledVector(travel,u),hit=closest.distanceToSquared(ship.position)<72;if(!h.near&&!hit&&closest.distanceToSquared(ship.position)<440){h.near=true;hostileNearMiss()}const sceneryHit=scenerySegmentHit(previous,h.mesh.position,1.5,2,false),blocked=h.mesh.position.y<=terrainHeight(h.mesh.position.x,h.mesh.position.z)+4||!!sceneryHit;if(hit&&!blocked){const p=h.mesh.position.clone();removeSamMissile(h);hostileMissileBurst(p);hitKick=Math.max(hitKick,1.15);flashScreen(.42);chirp(48,.18,.075);hitPlayer();return}if(blocked||h.life<=0){
...
function samExposure(s){
 ...
 if(range>maxRange||!samLineClear(s))return null;
 // The valley floor is the radar-clean kill lane. Low flight only buys safety when the pilot
 // deliberately tucks toward authored terrain; dead-center penetration is fast but exposed.
 const crossTrack=Math.abs(ship.position.x-valleyCenter(ship.position.z));
 const clutter=.30+.70*THREE.MathUtils.smoothstep(agl,16,95);
 const centerLane=1+.68*(1-THREE.MathUtils.smoothstep(crossTrack,110,310));
 const terrainTuck=.70+.30*THREE.MathUtils.smoothstep(crossTrack,210,430);
 const highExposure=1+THREE.MathUtils.smoothstep(agl,105,195)*1.05;
...
function updateSamNetwork(dt){
 ...
 for(const s of sam.sites){
  ...
  if(!c){
   ...
   if(s.lock>=1&&s.stage===2&&missionElapsed-s.lastCue>=1.1&&s.cooldown<=0&&sam.missiles.length<(mission.destroyed?2:1)&&missionElapsed-sam.lastLaunch>=3.2)launchSam(s);
  }
