...
const destinations=[
 ...
 {name:'NORTH RIDGELINE',position:new THREE.Vector3(380,0,-6000)},
...
function placeDestinations(){
 ...
 alpineRelay.position.set(380,terrainHeight(380,-3000)+100,-3000);alpineRelay.visible=worldIndex===1;
...
updateGuidance=function(){
 ...
 const challenge=worldIndex===1&&!relayPass.cleared;
 const pos=bandit?enemy.position:radar?encounter.targetPos:challenge?alpineRelay.position:destination.position;
 ...
 const owner=rear?'rear':bandit?'bandit':radar?'radar':challenge?'challenge':'destination';
 ...
 const label=rear?'ON YOUR SIX':bandit?'CONTACT':radar?(encounter.tracking?'RADAR PAINT':'RADAR'):challenge?'HIGH PASS RELAY':destination.name;
