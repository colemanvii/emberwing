...
const encounter={
  kind:null,phase:'idle',age:0,timer:0,offered:false,callout:false,
  group:null,wreck:null,dishPivot:null,beacon:null,targetPos:new THREE.Vector3(),
  lastRange:0,fx:[]
};
...
function resetEncounter(allowAgain=false){
  ...
  encounter.callout=false;encounter.lastRange=0;
...
function makeRadarInstallation(){
  ...
  dish.scale.y=.42;dish.rotation.x=Math.PI*.1;pivot.add(dish);
  ...
  encounter.dishPivot=pivot;encounter.beacon=beacon;
  return g;
}
...
function activateRadarOpportunity(){
  encounter.phase='active';encounter.age=0;encounter.callout=false;
...
function groundStrikeImpact(){
  ...
  for(let i=0;i<16;i++){
    const mesh=new THREE.Mesh(new THREE.SphereGeometry(1.8+Math.random()*2.4,8,6),new THREE.MeshBasicMaterial({
      color:i%3?0x2b2927:0x4b4038,transparent:true,opacity:.42,depthWrite:false
    }));
    mesh.position.copy(pos).add(new THREE.Vector3((Math.random()-.5)*18,Math.random()*7,(Math.random()-.5)*18));
    scene.add(mesh);
    const v=new THREE.Vector3((Math.random()-.5)*5,4+Math.random()*8,(Math.random()-.5)*5);
    const life=4.6+Math.random()*3.1;combatFX.push({mesh,v,life,maxLife:life,smoke:true,baseOpacity:.42,grow:.34+Math.random()*.28});
  }
  for(let i=0;i<10;i++){
...
function updateEncounter(dt){
  ...
  if(encounter.phase==='active'){
    ...
    if(encounter.age>24||(encounter.age>7&&range>1750))finishRadarOpportunity('lost');
    return;
  }
...
updateGuidance=function(){
  ...
  guideX=THREE.MathUtils.lerp(guideX,x,ons?.5:.28);guideY=THREE.MathUtils.lerp(guideY,y,ons?.5:.28);
  targetUI.style.left=guideX+'px';targetUI.style.top=guideY+'px';targetUI.style.opacity=lockState===2?'1':lockState===1?'.84':'.72';
  const ft=Math.max(0,Math.round(displayRangeFeet/10)*10);
  targetUI.dataset.range='RADAR · '+ft+' FT';
  targetUI.className=!ons?'offscreen':lockState===2?'lock':lockState===1?'track':'';
};
...
updateWeapons=function(dt){
  ...
  if(closest.distanceToSquared(encounter.targetPos)<144||strike.mesh.position.y<=terrain+2.5)groundStrikeImpact();
};
